...
Bakes the coverage_scope allowlist and baseline-objects manifest into the
reporter launcher so no manual --ignore_filename_regex flags are needed.
The workspace root is auto-detected by reporter.py via Path.cwd().resolve().
"""
...
""".format(
    ...
    runfiles = ctx.runfiles(
        files = [reporter, allowlist, baseline_objects],
...
reporter_wrapper = rule(
    ...
    attrs = {
        "reporter": attr.label(
            executable = True,
            cfg = "exec",
            doc = "The underlying reporter py_binary.",
        ),
        "coverage_scope": attr.label(
            cfg = "target",
            doc = "The coverage_scope target that provides the allowlist and objects manifest.",
        ),
    },
)

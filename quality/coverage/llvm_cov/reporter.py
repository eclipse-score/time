...
def main() -> None:
    ...
    if not workspace_root:
        # In the coverage sandbox cwd is the execroot; resolve() follows symlinks
        # to the real workspace directory on the host filesystem.
    ...
    llvm_profdata = r.Rlocation("llvm_toolchain/llvm-profdata")
    ...
    # --empty-profile was removed in LLVM 19; attempt to generate an equivalent
    # empty profile for baseline coverage (compiled-but-untested files at 0%).
    # If llvm-profdata rejects the call, baseline coverage is skipped gracefully.
    _empty_path = Path.cwd() / "empty.profdata"
    _empty_result = subprocess.run(
        [llvm_profdata, "merge", "--sparse", "--output", str(_empty_path)],
        stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True,
    )
    empty_profdata: Optional[str] = str(_empty_path) if _empty_result.returncode == 0 else None
    if empty_profdata is None:
        print("WARNING: Could not create empty profile — baseline coverage (0% for "
              "untested compiled files) will be skipped.", file=sys.stderr)

    # Load baseline objects (production library archives) for zero-coverage baseline.
    baseline_objects = load_baseline_objects(
        r, args.baseline_objects, workspace_root)
    ...
    if args.coverage_allowlist:
        ...
        if allowlist_files:
            ...
            baseline_files = set()
            if baseline_objects and empty_profdata is not None:
                baseline_files = get_covered_files(
                    llvm_bin_path, baseline_objects, empty_profdata, workspace_root)
            ...
            print("ERROR: Coverage allowlist is empty, falling back to filter_regexes.txt.",
                  file=sys.stderr)
            sys.exit(-1)
    elif args.ignore_filename_regex:
        filter_regexes = args.ignore_filename_regex
        print(f"INFO: Applying {len(filter_regexes)} --ignore-filename-regex exclusion pattern(s).",
              file=sys.stderr)
    common_args = {
    ...
    if baseline_only_archives and empty_profdata is not None:
        baseline_lcov_args = {
            "llvm_bin_path": llvm_bin_path,
            "objects": baseline_only_archives,
            "instr_profile": empty_profdata,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ignore_filename_regex", action="append", default=[],
                        help="Path regex to exclude from the report (repeatable). Used when no allowlist is set.")
    ...
    parser.add_argument("--workspace_root", type=str, default=None,
                        help="Real workspace root. Auto-detected from MODULE.bazel if not set.")

...
TEST_F(SharedMemTimeBaseWriterTestFixture, Writestd::string_view_ToClosedWriter_Fails) {
    // Act
...
TEST_F(SharedMemTimeBaseWriterTestFixture, Writestd::string_view_ToOpenWriter_Succeeds) {
    // Act
    tb_writer_->Open();
...
TEST_F(SharedMemTimeBaseWriterTestFixture, Writestd::string_view_OnSizeFieldOutOfBounds_Fails) {
    // Act
    tb_writer_->Open();
...
TEST_F(SharedMemTimeBaseWriterTestFixture, Writestd::string_view_OnDataOutOfBounds_Fails) {
    // Act
    tb_writer_->Open();
    tb_writer_->SetPosition(tb_writer_->GetMaxSize() - 20u);
...
TEST_F(SharedMemTimeBaseWriterTestFixture,
    ...
    static constexpr const std::byte span_data[] = {1, 2, 3};
    tb_writer_->Open();
...
TEST_F(SharedMemTimeBaseWriterTestFixture, WriteUserData_OnIllegalSize_Succeeds) {
    ...
    std::array<std::byte, 5u> data = {1, 2, 3, 4, 5};
    tb_writer->Open();
    tb_writer->lock();

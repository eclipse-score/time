...
def analyze_database(
    ...
    if analysis_report_path and os.path.exists(analysis_report_path):
        ...
        try:
            ...
            # The deviation queries (ListDeviationRecords.ql etc.) import
            # 'codingstandards.*' libraries. The compiled pack bundles these
            # under .codeql/libraries/. Copy them into the CodeQL global
            # package cache (~/.codeql/packages/) so that 'codeql database
            # run-queries' inside analysis_report resolves imports locally
            ...
            try:
                ...
                if os.path.isdir(libraries_path):
                    for scope in os.listdir(libraries_path):
                        scope_src = os.path.join(libraries_path, scope)
                        if not os.path.isdir(scope_src):
                            continue
                        for pkg in os.listdir(scope_src):
                            pkg_src = os.path.join(scope_src, pkg)
                            pkg_dst = os.path.join(codeql_pkg_cache, scope, pkg)
                            if os.path.isdir(pkg_src) and not os.path.exists(pkg_dst):
                                os.makedirs(os.path.dirname(pkg_dst), exist_ok=True)
                                shutil.copytree(pkg_src, pkg_dst)
                                print(f" Installed pack into cache: {scope}/{pkg}")
            except Exception as install_err:
                print(f" Warning: could not pre-install library packs: {install_err}")

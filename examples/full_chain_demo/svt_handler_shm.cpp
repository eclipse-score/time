...
// Original location: score/time_daemon/src/application/svt/svt_handler_shm.cpp
...
SvtHandlerShm::SvtHandlerShm() noexcept
    : job_runner_{nullptr},
      msg_broker_{nullptr},
      ...
      handler_status_{TimebaseHandler::Status::kIdle}
{
    ...
    score::mw::log::LogInfo(kTimeBaseHandlerSvt) << "SvtHandlerShm created!";
}

void SvtHandlerShm::Initialize() noexcept
{
    ...
    score::mw::log::LogInfo(kTimeBaseHandlerSvt) << "SvtHandlerShm msg broker initialized!";
}

void SvtHandlerShm::RunOnce(const score::cpp::stop_token& token) noexcept
{
    switch (handler_status_)
    {
        case TimebaseHandler::Status::kIdle:
        {
            ...
            score::mw::log::LogInfo(kTimeBaseHandlerSvt) << "SvtHandlerShm initialization started";
            break;
        }
            ...
            switch (status)
            {
                case JobRunner::Result::kSucceed:
                {
                    score::mw::log::LogInfo(kTimeBaseHandlerSvt)
                        << "SvtHandlerShm initialization done, starting proactive machines!";
                    job_runner_.reset();
                    ctrl_flow_divider_->Start();
                    gptp_machine_->Start();
                    handler_status_ = TimebaseHandler::Status::kWorking;
                    score::mw::log::LogInfo(kTimeBaseHandlerSvt) << "SvtHandlerShm is working";
                    break;
                }
                case JobRunner::Result::kFailed:
                {
                    handler_status_ = TimebaseHandler::Status::kFailed;
                    score::mw::log::LogError(kTimeBaseHandlerSvt) << "SvtHandlerShm initialization failed!!";
                    break;
                }
...
void SvtHandlerShm::Stop() noexcept
{
    if (handler_status_ == TimebaseHandler::Status::kWorking)
    {
        gptp_machine_->Stop();
        ctrl_flow_divider_->Stop();
        score::mw::log::LogInfo(kTimeBaseHandlerSvt) << "SvtHandlerShm stopping proactive machines!";
    }
}

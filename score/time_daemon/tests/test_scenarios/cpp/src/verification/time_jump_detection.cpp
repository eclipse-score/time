...
score::td::PtpTimeInfo make_jump_frame(std::uint16_t seq_id)
{
    ...
    data.sync_fup_data.sync_ingress_timestamp = kBaseIngress + static_cast<std::uint64_t>(seq_id) * kFrameStep;
    data.sync_fup_data.precise_origin_timestamp =
        kBaseOrigin + static_cast<std::uint64_t>(seq_id) * kFrameStep + 1'000'000ULL;  // +1 ms extra on origin side
    return data;
}
}  // namespace
...
void TimeJumpDetection::run(const std::string& /*input*/) const
{
    ...
    TRACING_INFO(
        ...
        std::pair{std::string{"time_jump_future_detected"}, std::string{time_jump_future_detected ? "true" : "false"}});
}

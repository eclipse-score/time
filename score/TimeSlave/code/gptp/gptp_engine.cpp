...
bool GptpEngine::Initialize()
{
    ...
    if (!identity_->Resolve(opts_.iface_name))
    {
        score::mw::log::LogError(score::td::kGPtpMachineContext)
            << "GptpEngine: failed to resolve ClockIdentity for "
            << opts_.iface_name;
        return false;
    }

    pdelay_ = std::make_unique<PeerDelayMeasurer>(
        identity_->GetClockIdentity());
    ...
    if (!socket_->EnableHwTimestamping())
    {
        score::mw::log::LogWarn(score::td::kGPtpMachineContext)
            << "GptpEngine: HW timestamping not available on "
    ...
    {
        score::mw::log::LogError(score::td::kGPtpMachineContext)
            << "GptpEngine: failed to create RxThread";
    ...
    {
        score::mw::log::LogError(score::td::kGPtpMachineContext)
            << "GptpEngine: failed to create PdelayThread";
        (void)Deinitialize();
        return false;
    }
    pdelay_started_ = true;

    score::mw::log::LogInfo(score::td::kGPtpMachineContext)
        << "GptpEngine initialized on " << opts_.iface_name;
    return true;
}
...
bool GptpEngine::ReadPTPSnapshot(score::td::PtpTimeInfo& info)
{
    ...
    const std::int64_t timeout_ns =
        static_cast<std::int64_t>(opts_.sync_timeout_ms) * 1'000'000LL;
...
void GptpEngine::PdelayLoop() noexcept
{
    ...
    const std::int64_t warmup_ns =
        static_cast<std::int64_t>(opts_.pdelay_warmup_ms) * 1'000'000LL;
    const std::int64_t next_warmup_ns =
        static_cast<std::int64_t>(next.tv_sec) * 1'000'000'000LL +
        next.tv_nsec + warmup_ns;
    ...
    const std::int64_t interval_ns =
        static_cast<std::int64_t>(
            opts_.pdelay_interval_ms > 0 ? opts_.pdelay_interval_ms : 1000)
        * 1'000'000LL;
...
void GptpEngine::HandlePacket(const std::uint8_t* frame, int len,
                               const ::timespec& hwts) noexcept
{
    ...
    const TmvT hw_ts{
        static_cast<std::int64_t>(hwts.tv_sec) * 1'000'000'000LL + hwts.tv_nsec};
    ...
    switch (msg.msgtype)
    {
            ...
            msg.parseMessageTs = TimestampToTmv(msg.follow_up.preciseOriginTimestamp);
            {
                ...
                if (result.has_value() && pdelay_)
                {
                    ...
                    if (pdr.valid)
                    {
                        ...
                        result->sync_fup_data.pdelay =
                            static_cast<std::uint64_t>(pdr.path_delay_ns);
                    }
                    else
                    {
        ...
        case kPtpMsgtypePdelayResp:
            msg.recvHardwareTS = hw_ts;
            msg.parseMessageTs =
                TimestampToTmv(msg.pdelay_resp.responseOriginTimestamp);
            if (pdelay_)
        ...
        case kPtpMsgtypePdelayRespFollowUp:
            msg.parseMessageTs =
                TimestampToTmv(msg.pdelay_resp_fup.responseOriginReceiptTimestamp);
            if (pdelay_)
...
void GptpEngine::UpdateSnapshot(const SyncResult&  sync,
                                 const PDelayResult& pdelay) noexcept
{
    std::lock_guard<std::mutex> lk(snapshot_mutex_);

    const std::int64_t local_rx_ns =
        static_cast<std::int64_t>(sync.sync_fup_data.reference_local_timestamp);
    ...
    snapshot_.status.is_correct =
        !sync.is_time_jump_future && !sync.is_time_jump_past;

 ...
 ********************************************************************************/
#ifndef SCORE_TIMESLAVE_CODE_GPTP_DETAILS_SYNC_STATE_MACHINE_H
...
struct SyncResult
{
    ...
    score::td::SyncFupData  sync_fup_data{};    ///< Ready to copy into PtpTimeInfo (pdelay field filled by engine)
...
class SyncStateMachine final
{
  public:
    ...
    explicit SyncStateMachine(
        std::int64_t jump_future_threshold_ns = 500'000'000LL) noexcept;
    ...
    bool IsTimeout(std::int64_t mono_now_ns,
                   std::int64_t timeout_ns) const;

    /// @return The latest computed neighborRateRatio (1.0 until first pair).
    double GetNeighborRateRatio() const { return neighbor_rate_ratio_; }

  private:
    SyncResult BuildResult(const PTPMessage& sync,
...
#endif  // SCORE_TIMESLAVE_CODE_GPTP_DETAILS_SYNC_STATE_MACHINE_H
